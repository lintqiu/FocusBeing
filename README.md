...
- **方式二**：设置环境变量，免每次输入（将下面的 `你的accessKeyId`、`你的accessKeySecret` 替换为控制台复制的真实值）：
  ...
  **Windows（永久：用户环境变量）**：  
  设置 → 系统 → 关于 → 高级系统设置 → 环境变量 → 用户变量 → 新建：变量名 `ACCESS_KEY_ID`，变量值填 accessKeyId；再新建 `ACCESS_KEY_SECRET`，变量值填 accessKeySecret。
...
FocusBeing, 数字人, 数字人生成, AI数字人, 视频生成, ai视频, 语音合成, 唇形同步
...
- 在终端中执行（将 `https://github.com/lintqiu/FocusBeing` 替换为实际技能仓库地址，例如 `https://github.com/lintqiu/FocusBeing`）：
  ```bash
  npx skills add https://github.com/lintqiu/FocusBeing -s FocusBeing -y
  ```
- 参数说明：`-s FocusBeing` 为技能名称，`-g` 表示按全局/规范安装，`-y` 表示默认确认、非交互。
- 安装完成后，技能会出现在 OpenClaw 的本地技能目录（具体路径由 OpenClaw 约定，如用户目录下的 `.openclaw/workspace/skills/FocusBeing`）。
...
- **requests**：必须安装该库，否则无法发起 HTTP 请求。  
  - 推荐在仓库根目录：`pip install -r requirements.txt`  
  - 或单独安装：Linux / macOS 使用 `pip3 install requests`；Windows 使用 `pip install requests`  
...
## 故障排除：PyCharm 提示 Invalid Python SDK

**原因**：IDE 若绑定到**其他项目**的 Python 虚拟环境（例如已删除、移动或损坏路径下的 `venv\Scripts\python.exe`），会报「SDK 无效」。本仓库的 `.idea` 不应再指向外部技能目录下的解释器。
...
- Python 3.6+
- `requests`：`pip install -r requirements.txt` 或 `pip install requests`
