...
def _read_credentials_from_env():
    """从环境变量读取一对凭证。支持 ACCESS_KEY_* 与 FOCUSAVATAR_ACCESS_KEY_*（可混用）。"""
    access_key_id = (
...
def get_credentials():
    ...
    说明：在 IDE「运行」、助手代跑脚本、或 stdin 被重定向时，input() 往往立刻得到空字符串，
    并非「没输入」，而是当前进程没有可用的交互式终端。请在本机终端运行或改用环境变量。
    """
    access_key_id, access_key_secret = _read_credentials_from_env()
    ...
    # 未从环境变量拿到完整凭证：先引导用户去控制台获取，再交互输入
    ...
    if not access_key_id:
        access_key_id = _prompt_until_non_empty(
            "请输入 accessKeyId（本脚本直接读取，模型看不到）: ",
            "accessKeyId",
        )
    if not access_key_secret:
        access_key_secret = _prompt_until_non_empty(
            "请输入 accessKeySecret（本脚本直接读取，模型看不到）: ",
            "accessKeySecret",
        )
    print(f"\n✓ 密钥已读取，全程仅在本脚本内使用")
    print()
