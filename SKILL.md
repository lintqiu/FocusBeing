...
BASE_URL 由环境变量 `FocusBeing_api` 或与脚本兼容的 `FocusBeing_api` 指定（未设置时默认示例：`https://yunji.focus-jd.cn`）。
...
获取后可在使用技能时按提示输入，或设置环境变量 `ACCESS_KEY_ID`、`ACCESS_KEY_SECRET` 免输入。
...
- **需要网络访问**：会访问环境变量 `FocusBeing_api`（或 `FocusBeing_api`）指定的后端（未设置时使用默认地址），请确保本机可访问该地址（防火墙、代理允许出站）。

---

## 故障排除：PyCharm Invalid Python SDK

若 IDE 绑定到已失效的**其他项目**虚拟环境路径，会报 SDK 无效。请在 **设置 → 项目 → Python 解释器** 中改选本机 Python 或本仓库下的 `.venv\Scripts\python.exe`，并执行 `pip install -r requirements.txt`。凭证推荐使用 `FOCUSAVATAR_ACCESS_KEY_ID` / `FOCUSAVATAR_ACCESS_KEY_SECRET`，脚本亦兼容 `ACCESS_KEY_ID` / `ACCESS_KEY_SECRET`。
...
- 所有请求**直接发往后端**（由 `FocusBeing_api` 指定），数据处理与存储由该后端及所属方负责。
